...
public class DataDAO {
    ...
    public static int hideFile(Data file)throws SQLException, IOException {
        ...
        if (!f.exists() || !f.canRead()) {
            System.out.println("File does not exist or cannot be read: " + f.getAbsolutePath());
            return -1;  // Handle the error appropriately
        }
        long maxSize = 1073741824; // 1 GB
        if (f.length() > maxSize) {
            System.out.println("File is too large to store in database: " + f.getAbsolutePath());
            return -1;
        }
